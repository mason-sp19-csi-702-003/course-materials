---
title: "Homework 1 — Optimize Matrix Multiplication"
date: "Due: February 18, 2019 @ 11:59pm"
---
...
Your task in this assignment is to write an optimized [matrix multiplication][matmul-wiki] function for XSEDE's [Bridges][cluster-documentation] supercomputer.
...
*   Your code must use [double-precision][double-precision-wiki] to represent real numbers.
    A common reference implementation for

*   Your code should be based on the 2*n*<sup>3</sup> algorithm.
    Submissions that implement algorithms outside this restriction, such as the [Strassen][strassen-wiki] algorithm, will not be graded.

*   You may not use compiler flags that automatically detect `dgemm` kernels and replace them with BLAS calls, i.e. Intel's [`-matmul`][intel-matmul-flag] flag.

*   The matrices are all stored in [column-major order][column-major-order], i.e. C<sub>i,j</sub> `== C(i,j) == C[(i - 1) + (j - 1) * n]`, for `i = 1:n`, where `n` is the number of rows in `C`.
...
*   We will check correctness by the following componentwise error bound,
    ...
    where ε = 2<sup>-52</sup> = 2.2 ⨉ 10<sup>-16</sup> is the [machine epsilon][machine-epsilon-wiki].
...
*   Please carefully read this section for implementation details and hints.
    Stay tuned to [Moodle][moodle-hw1-hints] for updates and clarifications, as well as discussion.
    If you are new to optimizing numerical codes, we recommend reading the papers in the [references](#references) section.

*   The GNU and Intel compilers can let you take advantage of Bridges' Intel(R) Xeon(R) CPU E5-2695 v3 @ 2.30GHz processors' [support for AVX2, AVX, SSE4.2, SSE4.1, SSSE3, SSE2, and SSE][cluster-intrinsics] intrinsic instructions.
    [Click here for a broad overview of what these various definitions mean][intrinsics-def].

*   To get good performance, you will need to manually vectorize your code using compiler intrinsics.
    [Intel provides a convenient applet containing an interactive, searchable list of all of the available intrinsics functions.][intel-intrinsics-applet]
...
*   One possible optimization to consider for the multiple tuning parameters in an optimized Matrix Multiplication code is autotuning in order to find the optimal/best available value.
    Libraries like [OSKI][oski] and [ATLAS][atlas-link] have shown that achieving the best performance sometimes can be done most efficiently by automatic searching over the parameter space.
    Some papers on this topic can be found on the [**Be**rkeley **B**enchmarking and **Op**timization (BeBOP) page][bebop-page]

### Other notes

*   You may use [any compiler available][cluster-compilers] on Bridges.
...
*   Double-precision matrix multiplication is the [`dgemm`][dgemm-definition] (**d**ouble-precision **ge**neral **m**atrix-**m**atrix multiply) routine in the level-3 [BLAS][blas].
...
*   Try complex numbers (single- and double-precision).
    [This forum thread][stackoverflow-complex-mult] gives advice on vectorizing complex multiplication with the conventional approach, but note that there are [other algorithms][complex-alt-algo] for this operation.

*   Optimize your matmul for the case when the inputs are symmetric.
    Consider [conventional][symmetric-matmul-conventional] and [packed][symmetric-matmul-packed] symmetric storage.
...
*   Goto, K., and van de Geijn, R. A. 2008. [Anatomy of High-Performance Matrix Multiplication][goto-paper], *ACM Transactions on Mathematical Software 34*, 3, Article 12.

    **Note:** explains the design decisions for the GotoBLAS `dgemm` implementation, which also apply to your code.

*   Chellappa, S., Franchetti, F., and Püschel, M. 2008. [How To Write Fast Numerical Code: A Small Introduction][chellappa-paper], *Lecture Notes in Computer Science 5235*, 196-259.
...
*   Bilmes, *et. al.* [The PHiPAC (Portable High Performance ANSI C) Page for BLAS3 Compatible Fast Matrix Matrix Multiply][bilmes-paper].
    ...
    **Note:** PHiPAC is a code-generating autotuner for matmul that started as a submission for this HW in a previous semester of CS267.
    Also see [ATLAS][atlas-link]; both are good examples if you are considering code generation strategies.

*   Lam, M. S., Rothberg, E. E, and Wolf, M. E. 1991. [The Cache Performance and Optimization of Blocked Algorithms][lam-paper], *ASPLOS'91*, 63-74.

    **Note:** clearly explains cache blocking, supported by with performance models.

*   [Notes][sse-helpful-notes] on vectorizing with SSE intrinsics and a short [video][hw1-helpful-video] by Brian Van Straalen.
    **Please ignore the due date on the last slide**

## Documentation:

*   [Bridges computing environment][cluster-documentation] documentation

*   [Intel Xeon Processor E5-2695 v3, 2.3GHz, 35M Cache][processor-documentation] documentation.

You are also welcome to learn from the source code of state-of-art BLAS implementations such as [GotoBLAS][gotoblas].
However, you should not reuse those codes in your submission.

[atlas-link]:                     http://math-atlas.sourceforge.net/
[bebop-page]:                     http://bebop.cs.berkeley.edu/
[bilmes-paper]:                   /references/Bilmes_1998_The_PHiPAC_v1.0_Matrix-Multiply_Distribution.pdf
[blas]:                           http://netlib.org/blas/index.html
[c++11-wiki]:                     https://en.wikipedia.org/wiki/C%2B%2B11
[c99-wiki]:                       http://en.wikipedia.org/wiki/C99
[chellappa-paper]:                /references/Chellappa_2008_How_to_Write_Fast_Numerical_Code.pdf
[cluster-compilers]:              https://portal.xsede.org/psc-bridges#programming
[cluster-documentation]:          https://portal.xsede.org/psc-bridges
[cluster-intrinsics]:             https://software.intel.com/en-us/node/523351
...
[complex-alt-algo]:               http://en.wikipedia.org/wiki/Multiplication_algorithm#Gauss.27s_complex_multiplication_algorithm
[dgemm-definition]:               http://www.netlib.org/blas/dgemm.f
[double-precision-wiki]:          http://en.wikipedia.org/wiki/Double-precision_floating-point_format
[doxygen-link]:                   https://www.stack.nl/~dimitri/doxygen/
[gcc-auto-vec]:                   http://gcc.gnu.org/projects/tree-ssa/vectorization.html
[gcc-compiler-complex]:           http://gcc.gnu.org/onlinedocs/gcc/Complex.html
[gcc-extensions]:                 http://gcc.gnu.org/onlinedocs/gcc/C-Extensions.html
[goto-paper]:                     /references/Goto_2008_Anatomy_of_High-performance_Matrix_Multiplication.pdf
[gotoblas]:                       http://www.tacc.utexas.edu/tacc-projects/gotoblas2
[how-to-make-tgz]:                https://kb.iu.edu/d/acfi
[hw1-helpful-video]:              https://www.cac.cornell.edu/vw/apc6/NotesOnHomework1.mp4
[intel-compiler-info]:            https://software.intel.com/en-us/compiler_15.0_ug_c
[intel-intrinsics-applet]:        https://software.intel.com/sites/landingpage/IntrinsicsGuide/
[intel-matmul-flag]:              http://software.intel.com/sites/products/documentation/hpc/composerxe/en-us/2011Update/fortran/win/copts/common_options/option_opt_matmul.htm
[interact-documentation]:         https://portal.xsede.org/psc-bridges#jobs:interactive
[intrinsics-def]:                 https://software.intel.com/en-us/articles/performance-tools-for-software-developers-intel-compiler-options-for-sse-generation-and-processor-specific-optimizations
[lam-paper]:                      /references/Lam_1991_The_Cache_Performance_and_Optimizations_of_Blocked_Algorithms.pdf
[machine-epsilon-wiki]:           http://en.wikipedia.org/wiki/Machine_epsilon
[matmul-wiki]:                    http://en.wikipedia.org/wiki/Matrix_multiplication
[moodle-hw1-hints]:               https://moodle.xsede.org/mod/forum/view.php?id=35
[oski]:                           http://bebop.cs.berkeley.edu/oski/
[processor-documentation]:        https://ark.intel.com/products/81057/Intel-Xeon-Processor-E5-2695-v3-35M-Cache-2_30-GHz
[restrict-keyword]:               http://cellperformance.beyond3d.com/articles/2006/05/demystifying-the-restrict-keyword.html
[scp-2factor]:                    https://portal.xsede.org/psc-bridges#transfer:tfa
[slurm-info]:                     https://slurm.schedmd.com/documentation.html
[sse-helpful-notes]:              /references/homework1notes_bvs.pdf
[stackoverflow-complex-mult]:     http://stackoverflow.com/questions/3211346/complex-mul-and-div-using-sse-instructions
[strassen-wiki]:                  http://en.wikipedia.org/wiki/Strassen_algorithm
[symmetric-matmul-conventional]:  http://www.netlib.org/lapack/lug/node122.html
[symmetric-matmul-packed]:        http://www.netlib.org/lapack/lug/node123.html
